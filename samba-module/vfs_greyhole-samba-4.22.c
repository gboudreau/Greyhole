/*
Copyright 2009-2022 Guillaume Boudreau, Edgars Binans
...
static int greyhole_openat(vfs_handle_struct *handle, const struct files_struct *dirfsp, const struct smb_filename *smb_fname, struct files_struct *fsp, const struct vfs_open_how *how);
...
static int greyhole_renameat(vfs_handle_struct *handle, files_struct *srcfsp, const struct smb_filename *oldname, files_struct *dstfsp, const struct smb_filename *newname, const struct vfs_rename_how *how);
...
static ssize_t greyhole_pwrite(vfs_handle_struct *handle, files_struct *fsp, const void *data, size_t count, off_t offset)
{
	...
	if (result >= 0) {
		...
		snprintf(filename, 44 + strlen(share) + nDigits(fsp_get_pathref_fd(fsp)) + 2*gnutls_hash_get_len(GNUTLS_DIG_SHA1), "/var/spool/greyhole/mem/%.0f-%s-%d-%s",
...
static ssize_t greyhole_recvfile(vfs_handle_struct *handle, int fromfd, files_struct *tofsp, off_t offset, size_t n)
{
	...
	if (result >= 0) {
		...
		char md5[2*gnutls_hash_get_len(GNUTLS_DIG_SHA1)+1];
		compute_hash(fname, md5);
		snprintf(filename, 44 + strlen(share) + nDigits(fsp_get_pathref_fd(tofsp)) + 2*gnutls_hash_get_len(GNUTLS_DIG_SHA1), "/var/spool/greyhole/mem/%.0f-%s-%d-%s",
			((double) (tp.tv_sec)*1000000.0),
			share,
			fsp_get_pathref_fd(tofsp),
			md5);
		spoolf = fopen(filename, "wt");
		fprintf(spoolf, "fwrite\n%s\n%d\n%s\n\n",
			share,
...
static int greyhole_renameat(vfs_handle_struct *handle, files_struct *srcfsp, const struct smb_filename *oldname, files_struct *dstfsp, const struct smb_filename *newname, const struct vfs_rename_how *how)
{
	...
	result = SMB_VFS_NEXT_RENAMEAT(handle, srcfsp, oldname, dstfsp, newname, how);
